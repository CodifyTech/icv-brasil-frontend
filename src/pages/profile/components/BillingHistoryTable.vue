...
import type { Invoice } from '@/pages/profile/types'
...
const invoices = computed((): Invoice[] => invoiceData.value)
    ...
    >
      ...
      <template #item.id="{ item }">
        <div class="text-body-1">
          <a :href="`billing-history/id/${item.id}`">
            #{{ item.id }}
          </a>
        </div>
      </template>
